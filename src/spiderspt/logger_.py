...
def init_logger() -> Logger:
    ...
    max_log_file = 40  # 最大日志文件数
    log_files: list[Path] = sorted(
        log_dir.glob("*.log"), key=lambda x: x.stat().st_ctime
    )  # 对日志文件进行排序
    # 删除多余的日志文件
    if len(log_files) > max_log_file:
        for old_log in log_files[: -(max_log_file - 1)]:
            old_log.unlink()
    ...
    # 添加文件日志处理器
    __logger.add(
        sink=log_dir / "{time:YYYY-MM-DD HH-mm-ss}.log",
        level="DEBUG",  # 级别
        format=__file_log_format,
        rotation="10 MB",  # 设置大小
        retention=max_log_file,  # 最多保留20个日志文件
        encoding="utf-8",
        enqueue=True,
        backtrace=True,  # 记录堆栈
        diagnose=True,  # 堆栈跟踪
